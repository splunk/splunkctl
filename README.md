...
2. Create a protected GitLab tag `v<version>` at that `main` commit, for
   example `v0.1.0`.
3. Run the manual `update:github` job for that tag. GitLab tests the validated
   source, creates the tag-based GitHub branch `gitlab-release/<tag>` from the
   tag projection, and opens a PR into protected GitHub `main`.
4. Run the manual `release:github` job for the same tag. It uploads the exact
   archives built by GitLab and targets the same validated publication commit.
5. Merge the publication PR when GitHub branch checks and approvals pass.
6. GitHub Actions verifies that the five GitLab-built release assets are
