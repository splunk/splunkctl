// Code generated by splunkctl codegen. DO NOT EDIT.
// Source: ../../cfg/bundles/default/static/splunkrc_cmds.xml
package generated

import (
	"github.com/spf13/cobra"
	"github.com/splunk/splunkctl/internal/client"
	"github.com/splunk/splunkctl/internal/cmdctx"
	"github.com/splunk/splunkctl/internal/output"
)

// HealthCmd is the top-level command for the health object.
var HealthCmd = &cobra.Command{
	Use:   "health",
	Short: "Manage health resources",
}

var HealthListCmd = &cobra.Command{
	Use:   "list",
	Short: `show the health report status of the Splunk instance.`,
	Example: `
  'splunkctl health list -features "BatchReader, TailReader"'`,
	Annotations: map[string]string{"describe_path": "/server/health/splunkd/details"},
	RunE: func(cmd *cobra.Command, args []string) error {
		c := clientFrom(cmd)
		if done, err := cmdctx.RunDescribe(cmd, c, cmd.Annotations["describe_path"]); done {
			return err
		}
		params := cmdctx.ExtraParams(cmd, map[string]string{})
		if v, _ := cmd.Flags().GetString("features"); v != "" {
			params["features"] = v
		}
		resourceID := ""
		results, err := c.Do(cmd.Context(), client.Request{
			Method: "GET",
			Path:   "/server/health/splunkd/details",
			ID:     resourceID,
			Params: params,
		})
		if err != nil {
			return err
		}
		outFlag, _ := cmd.Flags().GetString("output")
		return output.Print(cmd.Context(), results, outFlag)
	},
}

func init() {
	HealthCmd.AddCommand(HealthListCmd)
	HealthListCmd.Flags().String("features", "", `Comma separated feature names to be listed`)
}
