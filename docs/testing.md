...
the exact Linux AMD64 release binary. `package:github` archives those prebuilt
binaries with `VERSION` and `LICENSE`. `validate:github` does not run Go
commands: it checks each archive's member list, extracts it, compares every
file byte-for-byte with the build artifacts, then compares the public source
candidate with GitHub `main` and stores the checksummed validation artifact
consumed by the manual publish job.
...
the publication helper. The publication flow then has four jobs:

1. `package:github` builds the allowlisted candidate from the committed GitLab
   revision, scans it for secrets, and packages the prebuilt release binaries
   with `VERSION` and `LICENSE`.
2. `validate:github` checks the release archives against the exact build-stage
   files and creates an exact, checksummed source diff against GitHub `main`.
...
Temporary MR-only rules also expose packaging, archive validation,
`update:github`, and `release:github` for end-to-end testing. Each temporary CI
block is marked for removal before this change merges to `main`.
